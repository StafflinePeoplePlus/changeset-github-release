...
const resolveVersion = Either.match({
    onLeft: (packageJSONPath) => Package.readInfo(packageJSONPath).pipe(Effect.map((info) => info.version)),
    onRight: (version) => Effect.succeed(Package.Version(version)),
});
const provideOctokit = Effect.provideServiceEffect(Github.Octokit, Config.string('GITHUB_TOKEN').pipe(Effect.map((auth) => new Octokit({ auth }))));
...
const extract = Command.make('extract', { changelog, versionOrPackageJSON: Options.orElseEither(packageJSON, version) }, ({ changelog: changelogPath, versionOrPackageJSON }) => Effect.gen(function* () {
...
const checkRelease = Command.make('check-release', { owner, repo, versionOrPackageJSON: Options.orElseEither(packageJSON, version) }, ({ owner, repo: repoName, versionOrPackageJSON }) => Effect.gen(function* () {
    const version = yield* resolveVersion(versionOrPackageJSON);
    const tag = 'v' + version;
...
const skipOnExisting = Options.boolean('skip-on-existing').pipe(Options.withAlias('s'), Options.withDefault(false));
const release = Command.make('release', {
    ...
    skipOnExisting,
}, ({ owner, repo: repoName, changelog: changelogPath, versionOrPackageJSON, skipOnExisting }) => Effect.gen(function* () {
    const changelog = yield* Changelog.read(changelogPath);
    const version = yield* resolveVersion(versionOrPackageJSON);
    const entry = yield* Changelog.findEntry(changelog, version).pipe(Either.fromOption(() => `Changelog entry not found for version v${version}.`));
    const tag = 'v' + version;
    const repo = Github.repo(owner, repoName);
    const release = yield* Github.releaseByTagName(repo, tag);
    if (Option.isSome(release)) {
...
const command = Command.make('changeset-github-release').pipe(Command.withSubcommands([extract, checkRelease, release]));
